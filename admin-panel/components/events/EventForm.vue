<template>
    <form @submit.prevent="handleSubmit" class="space-y-4">
      ...
      <div>
        ...
        <textarea v-model="form.description" rows="4" class="w-full border px-3 py-2 rounded"></textarea>
      </div>
  
      <div>
        <label class="block text-sm font-medium text-gray-700">Image URL (optional)</label>
        <input v-model="form.imageUrl" type="url" class="w-full border px-3 py-2 rounded" />
      </div>
