...
export class PrayerService {
  ...
  async findAll(): Promise<Prayer[]> {
    const prayers: PrayerDocument[] = await this.prayerModel.find().lean().exec(); // ✅ Ensures plain objects
  ...
  async update(id: string, updatePrayerDto: UpdatePrayerDto): Promise<Prayer> {
    if (updatePrayerDto.timestamp) {
      updatePrayerDto.timestamp = new Date(updatePrayerDto.timestamp).toISOString(); // ✅ Ensure consistent ISO string format
    }
    ...
    if (!updatedPrayer) throw new NotFoundException(`Prayer with ID ${id} not found`);
  ...
  private transformPrayer(prayer: PrayerDocument | any): Prayer {
    ...
    const timestamp = prayer.timestamp instanceof Date ? prayer.timestamp : new Date(prayer.timestamp);
